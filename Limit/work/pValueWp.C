...
void pValueWp() {
	...
	for(auto iwpM = 0; iwpM < nM; ++iwpM){ 
		...
		double L3_N_exp_B = N_exp_B[iwpM] * 10;
                double L3_N_exp_S = N_exp_S[iwpM] * 10;

		double N_exp3_B = N_exp_B[iwpM] * 3;
                double N_exp3_S = N_exp_S[iwpM] * 3;

		L1_pvalue[iwpM]       = GetPvalue(N_exp3_B[iwpM], N_exp3_B[iwpM] + N_exp3_S[iwpM]);
		L1_significance[iwpM] = ROOT::Math::gaussian_quantile_c(L1_pvalue[iwpM],1);
		L1_exclusion[iwpM]    = GetExclusion(N_exp3_B[iwpM] + N_exp3_S[iwpM],N_exp3_B[iwpM]);
		//
		...
		cout << "L=3ab ,M=" << masses[iwpM] << " p=" << setw(13) << L1_pvalue[iwpM] << " sig=" << setw(10) << L1_significance[iwpM] << " exclu=" << L1_exclusion[iwpM] << endl;
		cout << "L=10ab ,M=" << masses[iwpM] << " p=" << setw(13) << L3_pvalue[iwpM] << " sig=" << setw(10) << L3_significance[iwpM] << " exclu=" << L3_exclusion[iwpM] << endl;
...
//	legend->AddEntry(gr_L1_pvalue       ,"L1_pvalue      " , "L"); 
	legend->AddEntry(gr_L3_significance ,"Lumi = 3 ab^{-1}" , "LP"); 
	legend->AddEntry(gr_L1_significance ,"Lumi = 1 ab^{-1}" , "LP"); 
